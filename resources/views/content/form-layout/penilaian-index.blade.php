...
@section('content')
    ...
    @if ($cpmi->penilaianHistori->isEmpty()) {{-- tampilkan hanya yang BELUM DIREKOMENDASIKAN --}}

                        <tr>
                            <td>{{ $cpmi->nama_cpmi }}</td>
                            <td>{{ $cpmi->tanggal_daftar }}</td>
                            <td>
                                @php $sudahRekom = $cpmi->penilaianHistori->count() > 0; @endphp
                                @if ($sudahRekom)
                                    <span class="badge bg-info">Sudah Direkomendasikan</span>
                                @elseif ($cpmi->penilaians->count() > 0)
                            ...
                            <td class="text-center align-middle">
                                ...
                                @if ($sudahRekom)
                                    <!-- CPMI sudah direkomendasikan, nonaktifkan aksi -->
            ...
            data-kriteria-{{ $kriteria->id }}="{{ $cpmi->penilaians->firstWhere('kriteria_id', $kriteria->id)?->subkriteria_id }}" @endforeach>
                                        <i class="bx bx-edit-alt"></i>
                                    </button>

                                    <!-- Tombol Hapus -->
                                    <form method="POST" class="d-inline delete-form"
                                        onsubmit="return confirm('Yakin ingin menghapus penilaian CPMI ini?')"
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    @if ($hasRange)
                                        <input type="number" name="penilaian[{{ $kriteria->id }}]"
                                            id="kriteria_{{ $kriteria->id }}" class="form-control" step="any" required
